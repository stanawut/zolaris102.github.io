<script>
	...
	let					bAttached			= false;
	let					bBind					= false;
	let					sDefaultTitle	=	'Main:HX';
	let					sActiveTitle	=	'';
	/* ---------------------------------------------------------------------------------------------- */
	export let data;
	console.log(" ------------ KDropdownCascaseList ------------ ",data.jsondata);
	...
	function handleSelectCat(e)
	{
		...
		console.log('KDDCC:etid:'+e.target.id);
...
{#if data}
	...
	<div class="btn-group mt-1" style="display:block">
		<button type="button" class="btn btn-outline-primary dropdown-toggle" style="width:100%" data-bs-toggle="dropdown" aria-expanded="false">
			{sActiveTitle}
		</button>
		...
		<ul class="dropdown-menu dropdown-menu-dark" style="width:100%">
				{#each data.jsondata as link,i}
						<li>
								<button id={link.id} css="none" class='dropdown-item' style="white-space:normal;width:100%" on:click={e=>handleSelectCat(e)} on:keypress={handleSelectCat}>{link.title}</button>
						</li>
				{/each}
		</ul>
	</div>

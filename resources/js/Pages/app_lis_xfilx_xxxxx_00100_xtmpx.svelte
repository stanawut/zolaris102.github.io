<script>
	...
	let sDebug = 'user_id | form:file_id | store_id | feuri'+" __|__ "+
								user_id+','+form_id+':'+file_id+','+store_id+"\n"+
								feuri.substring(40);
	...
	action_done: ""
	class_type: "fil"
	feuri: "http://localhost:5173/storage/app/public/12/venus/forms/18/document/pmn/12_18_frm.xml"
	file_id: "18"
	form_id: "18"
	...
	*/
	/* ---------------------------------------------------------------------------------------------- */
	function _fget_feuri(p_user_id,p_form_id,p_suffix,p_fext)
	{
		let fename = p_user_id+'_'+p_form_id+p_suffix+'.'+p_fext;
		return "http://localhost:5173/storage/app/public/"+p_user_id+
					 "/venus/forms/"+p_form_id+
					 "/document/pmn/"+fename;
	}
	/* ---------------------------------------------------------------------------------------------- */
	// load formfilled template created during create form {user_id}_{form_id}_frm.xml
	onMount( async () => {
			let akv_loadXml2Doc = {
															class_type		:'fil',
															_edit_cat			:'none',
															...
															form_id				:form_id,
															file_id				:form_id,
														//fname					:{user_id}_{form_id}_frm.xml this is UserForm derived form Form
															feuri					:feuri,
														//fxsdname			:{user_id}_{form_id}_xsd.xsd
														//store_id			:not use
			...
			akv_loadXml2Doc.feuri = _fget_feuri(user_id,form_id,'_frm','xml');								
			$sd_xid								= file_id;
			oXFil									= await LibTX.loadXml2Dog(akv_loadXml2Doc);
		//let feuri_xsd 				= 'http://localhost:5173/storage/app/public/{user_id}/venus/forms/{form_id}/document/pmn/{user_id}_{form_id}_xsd.xsd';
			let feuri_xsd					= _fget_feuri(user_id,form_id,'_xsd','xsd');
	...
	const handleSave = (e)=>{ 
															...
															let akvInputParams = 
																										{ 
																											...
																											store_id								: store_id							,
																											user_id									: user_id								,
																											fil_id									: ''										, // not exists until [create]
																											owner_id								: user_id								,
		...
		<div class="container">
			<header class="d-flex justify-content-center py-3">
				<ul class="nav nav-pills">
					<li class="nav-item"><button id="xditor_main_save" 	class="nav-link" on:click={e=>handleSave(e)} 	on:keypress={handleSave}>Create</button></li>
					<li class="nav-item"><a href="/gridboard" 					class="nav-link">Cancel</a></li>
				</ul>
			</header>
		</div>
